...
complexVector_t CreateDirectionalBfv (const Ptr<const ThreeGppAntennaArrayModel>& antenna,
                                      uint16_t sector, double elevation, bool debug)
{
  ...
  if (debug)
  {
  if (horizontalBeamStep.Get() == 9)
  {
    std::cout << "gNB: BeamformingVector CreateDirectionalBfv hAngle_radian: " << hAngle_radian << "rad. deg: " <<  hAngle_radian * (180.0 / M_PI) << std::endl;
    std::cout << "gNB: horizontalBeamStep is " << horizontalBeamStep.Get() << std::endl;
    std::cout << "gNB: sector is " << sector << std::endl;
    std::cout << "gNB: elevation is " << elevation << std::endl;
  }
  else
  {
    std::cout << "UE: BeamformingVector CreateDirectionalBfv hAngle_radian: " << hAngle_radian << "rad. deg: " <<  hAngle_radian * (180.0 / M_PI) << std::endl;
    std::cout << "UE: horizontalBeamStep is " << horizontalBeamStep.Get() << std::endl;
    std::cout << "UE: sector is " << sector << std::endl;
    std::cout << "UE: elevation is " << elevation << std::endl;
  }
  }
  ...
  for (auto ind = 0; ind < size; ind++)
    ...
    // std::cout << "bfv::CreateDirectionalBfv antenna element location" << std::endl;
                                     ...
                                     const Ptr<const ThreeGppAntennaArrayModel>& antenna)
{
  ...
  std::cout << " CreateDirectPathBfv called" << std::endl;
