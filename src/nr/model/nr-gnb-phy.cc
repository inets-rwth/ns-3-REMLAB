    ...
    m_n1Delay (4)
{
  ...
  m_sectorDegreeMap[{4.5, 13.5}] = 11;
  m_sectorDegreeMap[{13.5, 22.5}] = 12;
  m_sectorDegreeMap[{22.5, 31.5}] = 13;
  m_sectorDegreeMap[{31.5, 40.5}] = 14;
  m_sectorDegreeMap[{40.5, 49.5}] = 15;
  m_sectorDegreeMap[{49.5, 58.5}] = 16;
  m_sectorDegreeMap[{58.5, 67.5}] = 17;
  m_sectorDegreeMap[{67.5, 76.5}] = 18;
  m_sectorDegreeMap[{76.5, 85.5}] = 19;
  m_sectorDegreeMap[{85.5, 94.5}] = 20;
  m_sectorDegreeMap[{94.5, 103.5}] = 19;
  m_sectorDegreeMap[{103.5, 112.5}] = 18;
  m_sectorDegreeMap[{112.5, 121.5}] = 17;
  m_sectorDegreeMap[{121.5, 130.5}] = 16;
  m_sectorDegreeMap[{130.5, 139.5}] = 15;
  m_sectorDegreeMap[{139.5, 148.5}] = 14;
  m_sectorDegreeMap[{148.5, 157.5}] = 13;
  m_sectorDegreeMap[{157.5, 166.5}] = 12;
  m_sectorDegreeMap[{166.5, 175.5}] = 11;
  m_sectorDegreeMap[{175.5, 184.5}] = 10;
  m_sectorDegreeMap[{184.5, 193.5}] = 9;
  m_sectorDegreeMap[{193.5, 202.5}] = 8;
  m_sectorDegreeMap[{202.5, 211.5}] = 7;
  m_sectorDegreeMap[{211.5, 220.5}] = 6;
  m_sectorDegreeMap[{220.5, 229.5}] = 5;
  m_sectorDegreeMap[{229.5, 238.5}] = 4;
  m_sectorDegreeMap[{238.5, 247.5}] = 3;
  m_sectorDegreeMap[{247.5, 256.5}] = 2;
  m_sectorDegreeMap[{256.5, 265.5}] = 1;
  m_sectorDegreeMap[{265.5, 274.5}] = 0;
  m_sectorDegreeMap[{274.5, 283.5}] = 1;
  m_sectorDegreeMap[{283.5, 292.5}] = 2;
  m_sectorDegreeMap[{292.5, 301.5}] = 3;
  m_sectorDegreeMap[{301.5, 310.5}] = 4;
  m_sectorDegreeMap[{310.5, 319.5}] = 5;
  m_sectorDegreeMap[{319.5, 328.5}] = 6;
  m_sectorDegreeMap[{328.5, 337.5}] = 7;
  m_sectorDegreeMap[{337.5, 346.5}] = 8;
  m_sectorDegreeMap[{346.5, 355.5}] = 9;

  m_elevationDegreeMap[{0, -14.9999}] = 90;
  m_elevationDegreeMap[{-15, -44.9999}] = 120;
  m_elevationDegreeMap[{-45, -90}] = 150;
...
NrGnbPhy::UpdateUeSinrEstimate ()
{
  ...
  if (!IAisPerformed || m_omniFallback)
  {
    ...
    for (std::map<uint64_t, Ptr<NetDevice> >::iterator ue = m_ueAttachedImsiMap.begin (); ue != m_ueAttachedImsiMap.end (); ++ue)
    {
      ...
      Ptr<NrUePhy> uePhy; // unused?

      // For this UE IMSI we have this Power spectral density
...
{
  NS_LOG_FUNCTION (this);
  auto ctrlMsgs = PopCurrentSlotCtrlMsgs (); // list of ctrl msgs in the beginning of the queue
                                ...
                                const std::shared_ptr<DciInfoElementTdma> &dci)
{
  ...
  if (IAisPerformed && m_omniFallback)
  {
    // Change gNB antenna to omnidirectional mode 
    ...
    for (uint8_t i = 0; i < m_deviceMap.size (); i++)
    {
      ...
      if (ueDev->GetTargetEnb () != 0)
      {
        ...
        // Change UE antenna to omnidirectional mode
        ...
        if (maxSNRBeamId != m_beamManager->GetBeamId (m_ueAttachedImsiMap.at(imsiOfCSI)))
          {
            ...
            BeamformingVector maxSNRBfv = BeamformingVector (
              ...
              maxSNRBeamId.GetElevation (), true),
              maxSNRBeamId
            );
...
void NrGnbPhy::DoUpdateSuboptimalCSIRSBeams(uint64_t imsi, uint16_t sector, double elevation)
{
  ...
  BeamformingVector maxCsiBfv = BeamformingVector(
      CreateDirectionalBfv(m_beamManager->GetAntennaArray(),
                           ...
                           maxCSIRSBeamId.GetElevation(), true),
      maxCSIRSBeamId);
  // saving the beamforming vector for particular device
