...
private:
  ...
  bool IAisPerformed; // true at construction time and never after.
  ...
  bool m_firstrun; // not used
  bool m_IAcontinues; // not used
  bool m_csiTimer;
  double m_gnbElevationAngleStep; // set from attributes
  double m_gnbHorizontalAngleStep; // set from attributes
  uint16_t m_noOfInitialAccessUes = 0; // number of UEs performing IA right now. If > 0, don't stop beamSweep procedures.

  std::map<uint64_t, BeamId> m_imsiOptimalBeamId; // LAMM: might use/re-use this 
