  ...
  m_enbRrcSapProvider (0)
{
  ...
  std::cout << " New nrUeRrcProtocolIdeal instance created" << std::endl;

  // Set the parameters of the log-normal distribution
  double mean = 50.0;  // Mean of the underlying normal distribution
  double stdDev = 30.0; // Standard deviation of the underlying normal distribution

  m_logNormalLteDelay= CreateObject<LogNormalRandomVariable>();
  m_logNormalLteDelay->SetAttribute("Mu", DoubleValue(std::log(mean))); // Log of the mean
  m_logNormalLteDelay->SetAttribute("Sigma", DoubleValue(stdDev / mean)); // Standard deviation as a proportion of mean

  // FIXME: proper values? mean 250 will cause too large delay at all times and force UEs to sweep?
  double nMean = 250.0;
  double nStdDev = 200.0;
  m_normalLteDelay = CreateObject<NormalRandomVariable>();
  m_normalLteDelay->SetAttribute("Mean", DoubleValue(nMean)); // corresponds to the period of position change
  m_normalLteDelay->SetAttribute("Variance", DoubleValue(nStdDev));

  // Config::ConnectWithoutContext("LteRrcDelay", MakeCallback (&LogLteRrcDelay));
}
...
nrUeRrcProtocolIdeal::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::nrUeRrcProtocolIdeal")
    ...
    .AddAttribute("LteCoCellId",
                  "Cell ID of the LTE Coordinator",
                  UintegerValue (11),
                  MakeUintegerAccessor (&nrUeRrcProtocolIdeal::m_lteCellId),
                  MakeUintegerChecker<uint16_t> (std::numeric_limits<uint16_t>::min(), std::numeric_limits<uint16_t>::max()))
    .AddAttribute("EnableLteRrcDelay",
                  "Controls whether a delay will be applied for position reports which are sent over RRC messages",
                  BooleanValue(false),
                  MakeBooleanAccessor(&nrUeRrcProtocolIdeal::m_applyLteRrcDelay),
                  MakeBooleanChecker())
    .AddTraceSource("LteRrcDelay",
                    "Delay in milliseconds for the LTE RRC message transmission",
                    MakeTraceSourceAccessor (&nrUeRrcProtocolIdeal::m_lteRrcDelayTrace),
                    "ns3::nrUeRrcProtocolIdeal::LteRrcDelayTracedCallback")
  ;
  return tid;
}

void
nrUeRrcProtocolIdeal::LogLteRrcDelay(uint64_t imsi, int delay)
{
  *(m_lteDelayStream)->GetStream()
...
nrUeRrcProtocolIdeal::SetUeRrc (Ptr<LteUeRrc> rrc)
{
  ...
  // labf: for debug purposes. Logging of LTE delay 
  // m_lteDelayStream = asciiTraceHelper.CreateFileStream ("./../../../out/test/LteRrcDelay" + std::to_string (m_rrc->GetImsi()) + ".txt");
}
...
nrUeRrcProtocolIdeal::DoSetup (LteUeRrcSapUser::SetupParameters params)
{
  ...
  {
    std::cout << "nrUeRrcProtocolIdeal::DoSetup (LteUeRrcSapUser::SetupParameters params): Calling SetLteEnbRrcSapProvider" << std::endl;
...
// labf: send position report to the LTE CO
void
nrUeRrcProtocolIdeal::DoSendPositionReport (LteRrcSap::PositionReport msg)
{
    // m_enbRrcSapProvider is a nullptr until the UE connects to the network after sweeping
    if (m_lteEnbRrcSapProvider != nullptr)
    {
      // int milliseconds = std::round(m_logNormalLteDelay->GetValue());
      int milliseconds = 0;
      if (m_applyLteRrcDelay)
      {
        milliseconds = std::round(m_normalLteDelay->GetValue());
      }
      m_lteRrcDelayTrace(msg.imsi, milliseconds);
      std::cout << "LteRrcDelay:" << Simulator::Now().GetSeconds() << ","<< msg.imsi << "," << milliseconds << std::endl;
      Simulator::Schedule(ns3::MilliSeconds(milliseconds),
          &LteEnbRrcSapProvider::RecvPositionReport,
          //m_enbRrcSapProvider,  // <- this would be the serving gNB
          m_lteEnbRrcSapProvider, // <- this is the LTE coordinator
          m_rnti,
          msg);
    }
    // FIMXE: TODO: make the RRC_IDEAL_MSG_DELAY non zero or better use the actual PHY tx
    std::cout << "nrUeRrcProtocolIdeal::DoSendPositionReport : LteEnbRrcSapProvider::RecvPositionReport scheduled and executed" << std::endl;
}
...
nrUeRrcProtocolIdeal::DoTriggerRegisterUe (uint64_t imsi, const Ptr<NetDevice> &netDev)
{
  NS_LOG_FUNCTION (this);
  //test
  std::cout << "  + nrUeRrcProtocolIdeal::DoTriggerRegisterUe: for imsi " << static_cast<int>(imsi) << std::endl;
...
// This is not existing in upstream. Custom method to establish lte connection to the CO.
void
nrUeRrcProtocolIdeal::SetLteEnbRrcSapProvider ()
{
  std::cout << "nrUeRrcProtocolIdeal::SetLteEnbRrcSapProvider: a lot of stuff is going on here" << std::endl;
  NS_LOG_FUNCTION (this);

  NS_LOG_DEBUG ("RNTI " << m_rnti << " connected to cell " << m_lteCellId);
  std::cout << "nrUeRrcProtocolIdeal::SetLteEnbRrcSapProvider: RNTI " << static_cast<int>(m_rnti) << " connected to coordinator cell " << static_cast<int>(m_lteCellId) << std::endl;;
      ...
      for (int j = 0; 
           ...
           j++)
        {
          ...
          if (enbDev == 0)
            ...
            {
              if (enbDev->HasCellId (m_lteCellId))
                {
                  found = true;          
                  break;
                }
  ...
  NS_ASSERT_MSG (found, " Unable to find eNB with CellId =" << m_lteCellId);
  // m_lteEnbRrcSapProvider should be the Rrc SAP Provider of the lteCellId 11
  std::cout << "   nrUeRrcProtocolIdeal::SetLteEnbRrcSapProvider: enbDev that was found has cell id " << enbDev->GetCellId() << std::endl;
  ...
  std::cout << "   nrUeRrcProtocolIdeal::m_lteEnbRrcSapProvider addr is " << &m_lteEnbRrcSapProvider << std::endl;

  // TODO:: ESTABLISH THE REVERSE CONNECTION:i.e., FROM LTE ENB TO UE
  // apalovandov: that would be the reverse LTE connection from gNB to UE, not the CO->UE 
...
// This is called after "CSI-RS-RLM for IMSI: 1 , monitor beams from cell: 6, 6, 7, 9" lines
void 
nrUeRrcProtocolIdeal::DoSendSSBRSReport (LteRrcSap::UpdateBeamsTbRLM params)
{
  std::cout << "  +-+ nrUeRrcProtocolIdeal::DoSendSSBRSReport to m_lteEnbRrcSapProvider" << std::endl;
  Simulator::Schedule (RRC_IDEAL_MSG_DELAY,
                      &LteEnbRrcSapProvider::ForwardSSBRSBeamReport,
                      m_lteEnbRrcSapProvider,
                      params);
  // This is sent directly to coordinator and then distributed over X2 to gNBs.
  // Means that m_lteEnbRrcSapProvider is indeed the CO.
}
  ...
  :  m_enbRrcSapProvider (0)
{
  ...
  std::cout << " New NrGnbRrcProtocolIdeal instance created" << std::endl;
}
...
{
  std::cout << "NrGnbRrcProtocolIdeal::SetLteEnbRrcSapProvider: called and setting m_enbRrcSapProvider" << std::endl;
...
// labf
void
NrGnbRrcProtocolIdeal::DoSendRemBeamFromLTECoordinator(uint16_t rnti, LteRrcSap::LinkData linkData)
{
  // DOES NOTHING FOR NRGNBRRCPROTOCOLIDEAL
  // Only LTE RRC handles actual data transfer
  std::cout << "  ** NrGnbRrcProtocolIdeal::DoSendRemBeamFromLTECoordinator. does nothing" << std::endl;
}
