                                              ...
                                              uint16_t ccId) const
{
      ...
      for (uint16_t txSector = 0; txSector <= txNumRows; txSector++)
        {
          ...
          txPhy->GetBeamManager()->SetSector (txSector, txTheta); // TODO: how is rx sector defined
          ...
          for (double rxTheta = minRxTheta; rxTheta <= topRxTheta ; rxTheta = rxTheta + ueVerticalStep)
            {
              for (uint16_t rxSector = 0; rxSector <= rxNumRows; rxSector++) // TODO: how is rx sector defined
                {
  ...
  std::cout << "ideal-beamforming-algorithm.cc: CellScanBeamforming::DoGetBeamformingVectors: ";
  std::cout << "Beamforming vectors for gNB with node id: "<< gnbDev->GetNode()->GetId () <<
                ...
                " rx sector " << (M_PI * static_cast<double> (maxRxSector) / static_cast<double> (rxNumRows) - 0.5 * M_PI) / (M_PI) * 180 << std::endl;
