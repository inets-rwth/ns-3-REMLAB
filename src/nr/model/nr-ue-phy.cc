...
NrUePhy::NrUePhy ()
{
 ...
 // FIXME: fix intervals so that upper value is not included?
  ...
  m_sectorDegreeMap[{63, 81}] = 9;
  m_sectorDegreeMap[{81, 99}] = 10;
  m_sectorDegreeMap[{99, 117}] = 9;
  m_sectorDegreeMap[{117, 135}] = 8;
  m_sectorDegreeMap[{135, 153}] = 7;
  m_sectorDegreeMap[{153, 171}] = 6;
  m_sectorDegreeMap[{171, 189}] = 5;
  m_sectorDegreeMap[{189, 207}] = 3;
  m_sectorDegreeMap[{207, 225}] = 3;
  m_sectorDegreeMap[{225, 243}] = 2;
  m_sectorDegreeMap[{243, 261}] = 1;
  m_sectorDegreeMap[{261, 279}] = 0;
  m_sectorDegreeMap[{279, 297}] = 1;
  m_sectorDegreeMap[{297, 315}] = 2;
  m_sectorDegreeMap[{315, 333}] = 3;
  m_sectorDegreeMap[{333, 351}] = 4;
...
NrUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUePhy")
    ...
    .AddAttribute ("AmountOfGnbs", "Amount of gNBs in the simulation.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&NrUePhy::m_amountOfGnbs),
...
// This is the place where DL ctrl messages are recevied and processed
void
...
NrUePhy::UpdateSinrEstimate (uint16_t cellId, double sinr)
{
  ...
  // If the SINR data is being updated for the serving cell, we need to take a closer look at SINR level
...
NrUePhy::DoGenerateBeamVectorMap() 
{
  ...
  m_ssbRLMScanDirectionNumber = m_ueSectorNumber; // 33
  m_noOfTxSSBScanDirections = m_gnbSectorNumber; // 63
...
NrUePhy::ProcessSSBs (Ptr<NrPssMessage> pssMsg)
{
  ...
  if (m_IAperformed && pssMsg->GetDestinationImsi () == 0)
  {
    ...
    {
      // IF this is indeed an SSB that is in the second frame and so on..
      if (currentSfn.GetFrame () % (uint8_t)2 == 0 &&
          currentSfn.GetSubframe () == 0 &&
          currentSfn.GetSlot () == 0 &&
          pssMsg->GetSymbolOffset () == 0)
      {
        // .. then create an ssb processor for the current cell ID.
        ...
        // For the first RX sector, the first TX sector gives SNR evaluated to result of GetLastReceivedSNR
        m_cellIDSSBMap.at (cellId)->InsertBeamIdSNRPair (1, {1, m_spectrumPhy->GetLastReceivedSNR (cellId)});
        // Increment total amount of SSB counters per recevier
        m_txSSBCounterPerRx +=1;
      }  
    }
    // We already have an SSB processor for the current cell ID
    else
    {
      ...
      m_txSSBCounterPerRx += 1; // each new SSB for the currently set RX sector increments the counter

      // If we did not reach the last availabe RX sector
      if (rxSectorNumber < m_ueSectorNumber)
      {
        // And if the gNB sectors were not completely processed
        if (txSectorNumber <= m_gnbSectorNumber)
          {
            // Store info "for this RX sector we have this TX sector with such SNR"
            ...
            if (txSectorNumber == m_gnbSectorNumber)
              {
                ...
                if (m_txSSBCounterPerRx >= m_amountOfGnbs * m_gnbSectorNumber)
                {
                  ...
                  if (rxSectorNumber < m_ueSectorNumber - 1)
                  {
                    ...
                    // set nextRxBeamId to a beam on the next RX sector
                    nextRxBeamId = m_ueBeamVectorList.at (m_cellIDSSBMap.at (cellId)->rxSectorNumber + 1); 
                    m_beamTbSwept = nextRxBeamId; // assign it to the next beam that will receive all SSBs
                    // actually adjust the antenna by setting to the correct sector and elevation
                    m_beamManager->SetSector (nextRxBeamId.GetSector (), nextRxBeamId.GetElevation());
                  }
                  else
                  {
                    // reset the beam to the first one, as we reached the last possible RX sector?
                    nextRxBeamId = m_ueBeamVectorList.at (0);
                  }        
                  // Reset the counter that tracks how many SSBs the current RX sector received        
                  m_txSSBCounterPerRx = 0;
                }
                // We received beams from all TX sectors for the current RX sector.
                // Now increment the RX sector that will listen to SSBs and reset the TX sectors, 
                // as the TX will now sweep again for the new RX beam
                m_cellIDSSBMap.at (cellId)->rxSectorNumber++; 
                m_cellIDSSBMap.at (cellId)->txSectorNumber = 0;
              }
          }
      }
      // We reached the last RX sector, we have made a complete sweep at this point.
      // Reset everything
      else
      {
  ...
  else if (!m_IAperformed && m_ssbRlmOn) // This is exectued every 20ms after IA
  {
   ...
   // SSB processor exists for the selected cell ID
   else
   {
     // Select current RX and next TX sectors
     const uint16_t txSectorNumber = ++m_ssbRLMProcessorMap.at(cellId)->txSectorNumber;
     // Choose current rx sector number for the cell id
     const uint8_t rxSectorNumber = m_ssbRLMProcessorMap.at(cellId)->rxSectorNumber;

     // m_ssbRLMScanDirectionNumber is 33?
     if (rxSectorNumber < m_ssbRLMScanDirectionNumber) 
     {
       // m_noOfTxSSBScanDirectionsis 63 and never changes
       if (txSectorNumber <= m_noOfTxSSBScanDirections)
       {
         ...
         // if we scanned all TX sectors for the current RX sector
         if (txSectorNumber == m_noOfTxSSBScanDirections)
         {
           // Increment the RX sector number
           m_ssbRLMProcessorMap.at(cellId)->rxSectorNumber++;
           // Reset the TX sector number
           ...
           if (rxSectorNumber == m_ssbRLMScanDirectionNumber - 1)
           {
             ...
             {
               // Iterate over all SSB processors
                    ...
                    ++ssbRLMProcessorIter)
               {
                      ...
                      ++txBeamIter)
                 {
                   ...
                   // If we scanned for all TX directions, we break and completeReportTable remains on true
                   break;
                 }
               }
             }
             ...
             // If we scanned all possible combinations, our report can be considered complete.
             if (completeReportTable)
             {
               ...
               for (auto cellIndex = 1; cellIndex <= m_amountOfGnbs; cellIndex++)
               {
...
NrUePhy::RlmInitAfterRemIA()
{
  ...
  if (m_minCSIRSFromServingGnb != 4)
  {
    ...
    for (uint8_t n = 0; n < m_noOfBeamsTbRLM; n++)
    {
      tmp_beamsTbRLM.push_back(std::make_pair(n+1, BeamId{0,0.}));
    }
...
NrUePhy::RetrieveCellOptimalMap ()
{
  ...
  // For each gNB in the sim there is an entry in m_cellIDSSBMap.
  for (std::map<uint8_t, Ptr<SSBProcessor>>::iterator cellIterator = m_cellIDSSBMap.begin(); cellIterator != m_cellIDSSBMap.end(); ++cellIterator)
  {
    ...
    for (auto rlmIndex = 0; rlmIndex < m_noOfBeamsTbRLM; rlmIndex++)
    {
      ...
      listOfRLMBeams.emplace_back (std::pair<std::pair<SfnSf, uint16_t>, std::pair<double, BeamId>>(std::pair<SfnSf, uint16_t> (ssbProcessor->m_startingSfn, ssbProcessor->m_maxTxRxPairForCell.second.first),
                                    std::pair<double, BeamId> (ssbProcessor->m_maxSNRPerCell, ssbProcessor->m_maxTxRxPairForCell.second.second)));
      // for this imsi, we monitor beam from specific cell with this beam id.
...
NrUePhy::ProcessCSIRSs (Ptr<NrCSIRSMessage> csiRSMsg)
{
  if (!m_IAperformed) // If UE is still in IA, skip all of this
  {
    ...
    if (cellIdFromMsg == GetCellId () || m_minCSIRSFromServingGnb < 4)
    {
      ...
      // sfnSfKey index of CSI-RS message that icnudes the current frame remainder and subrframe and slot number
      SfnSfKey sfnSfKey = std::pair<uint8_t, std::pair<uint8_t, uint8_t>>(
          frameRemainder,
          std::pair<uint8_t, uint8_t>(currentSubframe,
                                      currentSlot));
      // If if key is not found -> sync error, clear map
      ...
      // Insert the UE currentRX beam ID (m_csiRXBeam), TX beam ID of gNB (csiRSMsg->GetTXBeamId().first)
      // and SNR of the received CSI-RS message
      ...
      // If all expected beams are received
      if (m_recvCSIMap.size() == m_noOfBeamsTbRLM)
      {
        // clear map at m_imsi and fill with empty vector
        m_csiRSTbReported.erase(m_imsi); // clear previous information
        ...
        for (uint8_t n = 0; n < m_noOfBeamsTbRLM; n++)
        {
          ...
          // Select the highest SNR value and up
          ...
          // store additional beams if primary beams are already checked
          else
          {
        ...
        // Now iterate over best beams
        // Update the beamforming vector to select the best beam for the serving cell.
        ...
        // Send back the info about each tracked beam
...
NrUePhy::DoSetRemBeam(LteRrcSap::LinkData linkData)
{
  ...
  if (m_IAperformed)
  {
    std::cout << Simulator::Now().GetSeconds() << ": UE received a REM beam during IA" << std::endl;  
    m_IAperformed = false;
    // Set bfv for the cell
    double bestRemElevation = linkData.bestAoAElevation;
    double bestRemAzimuth = linkData.bestAoAAzimuth;

    uint16_t remSector{0};
    double remElevation{0.};
    ...
    for (uint8_t cellId = 1; cellId <= m_amountOfGnbs; ++cellId)
    {
      Ptr<SSBProcessor> ssbProcessor = Create<SSBProcessor> ();
      ssbProcessor->m_cellId = cellId;
     // m_cellIDSSBMap holds cell ID as key and the ssbProcessor as value
    ...
    return; // can not simply execute rest of the code. m_registeredEnb(0) does not exist as cell is 0 while we are not connected
  }
  ...
  NS_ASSERT_MSG(m_registeredEnb.find(/*GetCellId()*/remCell) != m_registeredEnb.end(), "m_registeredEnb.find(remCell) was not found");
  complexVector_t currentBfvForGnb = m_beamManager->GetBeamformingVector(m_registeredEnb.at (/*GetCellId()*/remCell));
  ...
  if (remBfv.first == currentBfvForGnb)
    return; // we do not need to change anything for beam pointed to the current gNB
  ...
  params.imsi = m_imsi;
  params.m_beamSweepOrigin = BeamSweepTraceParams::UE_BEAM_ADJUSTMENT;
  params.foundCell = /*GetCellId()*/remCell;
  params.foundSector = remSector;
  params.foundElevation = remElevation;
