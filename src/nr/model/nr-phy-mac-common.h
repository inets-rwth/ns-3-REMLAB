...
//FIXME: this is probably the wrong pace for this structure
struct RemBeamSelectionTraceParams
{
  uint64_t imsi{0};
  uint8_t servingCellId{0};
  uint8_t bestCellId{0};
  BeamId idealPositionGnbRemBeam;
  BeamId idealPositionUeRemBeam;
  BeamId errPositionGnbRemBeam;
  BeamId errPositionUeRemBeam;
  int gnbSectorOffset{0};
  int gnbElevationOffset{0};
  int ueSectorOffset{0};
  int ueElevationOffset{0};
};

//FIXME: this is probably the wrong pace for this structure
struct RemQueryRecoveryAlgorithmTraceParams
{
  uint64_t imsi{0};
  uint8_t servingCellId{0};
  bool recoveryIsEnabled{false};
  bool remPointAvailableForQueriedPosition{false};
  bool remPointRecovered{false};
  double distanceBetweenReportedAndRecoveredPosition{0.};
  ns3::Vector reportedCoords{0., 0., 0.};
  ns3::Vector recoveredCoords{0., 0., 0.};
};
