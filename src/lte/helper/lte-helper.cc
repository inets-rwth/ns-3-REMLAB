...
void
LteHelper::LoadInventory(std::string inventory_file)
{
...
//test
void
LteHelper::LoadAndPassREMToEnbs(std::string input_files_folder)
{
  // Load the REM similar to how it is done in NrPhy::LoadInventory. Do this once
  // Pass it to each registered eNB. Pass references to avoid 11 copiesLoading the walk coordinates from: src/nr/model/Raytracing_UE_set/2760_cords.txt

  // Lte::Helper() must be called beforehand
  LoadInventory(input_files_folder + "Inventory_28GHz.txt");
  std::cout << "LteHelper::LoadAndPassREMToEnbs: REM loaded into LteHelper" << std::endl;
  std::cout << "LteHelper::LoadAndPassREMToEnbs: REM size " << m_allLinkData.size() << std::endl;

  // Now pass the REM to eNB RRC of the coordinator and only coordinator. We do not need REM at each gNB.

  //allLinkData now contains REM
