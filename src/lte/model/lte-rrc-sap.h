...
public:
  ...
  /// test: PositionReport from the UE
  struct PositionReport
  {
    uint64_t imsi{0};
    uint16_t cellId{0}; // source Cell ID
    Vector3D uePosition{0., 0., 0.};
  };
  ...
  struct LinkData
  {
    // FIXME: for now, we do not serialize the doubleVector_t members. Need to check if they bring any benefit.
    ...
    // doubleVector_t m_txId; // base station IDs
    // doubleVector_t m_delay; // propagation delay

    // This will be serialized
    uint16_t imsi;
    double bestTxId{0.};
    double bestAoDElevation{0.};
    double bestAoDAzimuth{0.};
    double bestAoAElevation{0.};
    double bestAoAAzimuth{0.};
    double bestPathloss{0.};
  };
...
public:
  ...
  //test
  virtual void SendPositionReport (PositionReport msg) = 0;
...
public:
  ...
  // labf 
  virtual void RecvRemBeamFromLTECoordinator(LteRrcSap::LinkData linkData) = 0;
};
...
public:
  ...
  virtual void SendRemBeamFromLTECoordinator (uint16_t rnti, LteRrcSap::LinkData linkData) = 0;
};
...
public:
 ...
 // test
  virtual void RecvPositionReport (uint16_t rnti, PositionReport msg ) = 0;
...
public:
  ...
  // test
  virtual void SendPositionReport (PositionReport msg);
...
template <class C>
void
MemberLteUeRrcSapUser<C>::SendPositionReport (PositionReport msg)
{
  m_owner->DoSendPositionReport (msg);
}
...
{
  // We get here from LteUeRrc::SetLteEnbRrcSapProviderFromHelper ()
  std::cout << "MemberLteUeRrcSapUser<C>::DoSetLteEnbRrcSapProviderFromHelper () called " << std::endl;
...
public:
  ...
  // labf
  virtual void RecvRemBeamFromLTECoordinator(LteRrcSap::LinkData linkData);
...
template <class C>
void
MemberLteUeRrcSapProvider<C>::RecvRemBeamFromLTECoordinator(LteRrcSap::LinkData linkData)
{
  Simulator::ScheduleNow (&C::DoRecvRemBeamFromLTECoordinator, m_owner, linkData);
}
...
public:
  ...
  // Location-aided beamforming
  virtual void SendRemBeamFromLTECoordinator (uint16_t rnti, LteRrcSap::LinkData linkData);
...
template <class C>
void
MemberLteEnbRrcSapUser<C>::SendRemBeamFromLTECoordinator (uint16_t rnti, LteRrcSap::LinkData linkData)
{
  std::cout << " * MemberLteEnbRrcSapUser<C>::SendRemBeamFromLTECoordinator for rnti " << (int)rnti << std::endl;
  // This has to be defined in nr-rrc-* and lte-rrc-*
  m_owner->DoSendRemBeamFromLTECoordinator(rnti, linkData); 
}
...
public:
  ...
  //test
  virtual void RecvPositionReport (uint16_t rnti, PositionReport msg);
...
template <class C>
void
MemberLteEnbRrcSapProvider<C>::RecvPositionReport (uint16_t rnti, PositionReport msg)
{
  //std::cout << "MemberLteEnbRrcSapProvider<C>::RecvPositionReport schedules a C::DoRecvPositionReport" << std::endl;
  Simulator::ScheduleNow (&C::DoRecvPositionReport, m_owner, rnti, msg);
}
