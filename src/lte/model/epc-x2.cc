...
EpcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  if (procedureCode == EpcX2Header::HandoverPreparation)
    {
      if (messageType == EpcX2Header::InitiatingMessage)
        {
          std::cout << "  * EpcX2::RecvFromX2cSocket: received a handover request" << std::endl;
  ...
  else if(procedureCode == EpcX2Header::UePositionReport)
    {
      std::cout << "EpcX2::RecvFromX2cSocket: received an UePositionReport " << std::endl;

      EpcX2UePositionReport x2UePositionReportHeader;
      packet->RemoveHeader(x2UePositionReportHeader);

      LteRrcSap::PositionReport params;
      params.uePosition = x2UePositionReportHeader.GetPositionReport().uePosition;
      params.cellId = x2UePositionReportHeader.GetPositionReport().cellId;
      params.imsi = x2UePositionReportHeader.GetPositionReport().imsi;

      m_x2SapUser->RecvUePositionReport (params);
    } 
  else if (procedureCode == EpcX2Header::LinkDataToGnb)
    {
      std::cout << "EpcX2::RecvFromX2cSocket: received a LinkDataToGnb" << std::endl;
      EpcX2LinkDataToGnbHeader x2LinkDataToGnbHeader;
      packet->RemoveHeader(x2LinkDataToGnbHeader);

      LteRrcSap::LinkData params;
      params = x2LinkDataToGnbHeader.GetLinkData();
      std::cout << "   EpcX2::RecvFromsocket. params aod elev: " << params.bestAoDElevation << std::endl;
      std::cout << "   EpcX2::RecvFromsocket. params aod azim: " << params.bestAoDAzimuth << std::endl;

      m_x2SapUser->RecvREMLinkDataToGnb(params);
    }
  else
    {
...
{
  std::cout << "  * EpcX2::DoSendHandoverRequest: called with " << std::endl; 

  std::cout << "  ** oldEnbUeX2apId = " << params.oldEnbUeX2apId << std::endl;
  std::cout << "  ** sourceCellId = " << params.sourceCellId << std::endl;
  std::cout << "  ** targetCellId = " << params.targetCellId << std::endl;
  std::cout << "  ** mmeUeS1apId  = " << params.mmeUeS1apId << std::endl;
  ...
  std::cout << "  ** sourceSocket = " << sourceSocket << std::endl;
  std::cout << "  ** targetIpAddr = " << targetIpAddr << std::endl;
  ...
  std::cout << "  ** Sent to targed IP addr " << std::endl;
}
...
EpcX2::DoSendUeSSBRSReport (LteRrcSap::UpdateBeamsTbRLM params)
{
  ...
  Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [params.targetCellId];
  // test
  std::cout << "EpcX2::DoSendUeSSBRSReport: targetCellId: "<< (unsigned)params.targetCellId << std::endl;
  Ptr<Socket> sourceSocket = socketInfo->m_localUserPlaneSocket;
  Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;
...
//test
void
EpcX2::DoSendPositionReportToCoordinator(EpcX2Sap::UePositionReport params)
{
  // For some reason cell 7 is not in the m_x2InterfaceSockets
  std::cout << "EpcX2::DoSendPositionReportToCoordinator: m_x2InterfaceSockets state: " << std::endl;
  for (auto socket : m_x2InterfaceSockets)
  {
    std::cout << socket.first << " " << socket.second->m_remoteIpAddr << std::endl;
  }
  std::cout << "   starting to search for key " << params.targetCellId << std::endl;

  NS_ASSERT_MSG (m_x2InterfaceSockets.find (params.targetCellId) != m_x2InterfaceSockets.end (),
                 "EpcX2::DoSendPositionReportToCoordinator: Missing infos for targetCellId = " << params.targetCellId);

  Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [params.targetCellId];
  Ptr<Socket> sourceSocket = socketInfo->m_localUserPlaneSocket;
  Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;

  EpcX2UePositionReport x2PositionReport;
  x2PositionReport.SetPositionReport(params.report);
  x2PositionReport.SetTargetCellId(params.targetCellId);
  x2PositionReport.SetUeImsi(params.imsi);

  EpcX2Header x2Header;
  x2Header.SetMessageType (EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (EpcX2Header::UePositionReport);
  x2Header.SetLengthOfIes (x2PositionReport.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2PositionReport.GetNumberOfIes ());

  Ptr<Packet> packet = Create <Packet> ();
  packet->AddHeader (x2PositionReport);
  packet->AddHeader (x2Header);
  sourceSocket->SendTo (packet, 0, InetSocketAddress (targetIpAddr, m_x2cUdpPort));
...
void 
EpcX2::DoSendREMLinkDataToGnb(EpcX2Sap::LinkDataToGnb params)
{
  std::cout << "EpcX2::DoSendREMLinkDataToGnb: sending best beam info to cell ID " << params.targetCellId << std::endl;
  NS_ASSERT_MSG (m_x2InterfaceSockets.find (params.targetCellId) != m_x2InterfaceSockets.end (),
                 "EpcX2::DoSendPositionReportToCoordinator: Missing infos for targetCellId = " << params.targetCellId);
  ...
  EpcX2LinkDataToGnbHeader x2LinkDataToGnbHeader;
  x2LinkDataToGnbHeader.SetLinkData(params);
  x2LinkDataToGnbHeader.SetTargetCellId(params.targetCellId);
  x2LinkDataToGnbHeader.SetUeImsi(params.imsi);

  EpcX2Header x2Header;
  x2Header.SetMessageType (EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (EpcX2Header::LinkDataToGnb);
  x2Header.SetLengthOfIes (x2LinkDataToGnbHeader.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2LinkDataToGnbHeader.GetNumberOfIes ());

  Ptr<Packet> packet = Create <Packet> ();
  packet->AddHeader (x2LinkDataToGnbHeader);
  packet->AddHeader (x2Header);
  sourceSocket->SendTo (packet, 0, InetSocketAddress (targetIpAddr, m_x2cUdpPort));
}
