...
//#include <optional> // Seems to be unavailable for current ns3 version. Required C++17
...
public: 
  ...
  //test
  void RecvPositionReport (LteRrcSap::PositionReport msg);
  /**
...
public:
  ...
  typedef void (*UePositionReportTracedCallback)
      (const UeCompletePositionReport completePositionReport);

  typedef void (*RemBeamSelectionTracedCallback)
      (const RemBeamSelectionTraceParams remBeamSelectionTrace);

  typedef void (*RemMeasurementErrorTracedCallback)
      (const std::map<Vector, std::pair<double, double>> remMeasurementError);

  typedef void (*RemNonEmptyEntriesTracedCallback)
      (const std::vector<Vector> remNonEmptyEntries);

  typedef void (*RemQueryRecoveryAlgorithmTracedCallback)
      (const RemQueryRecoveryAlgorithmTraceParams remQueryRecoveryTrace);
...
private:
  ...
  // flexRLM function that recieves the candidate best BPLs from the LTE eNB coordinator?
...
private:
  ...
  // flexRLM - initiate the sweep based on RLM
  ...
  /**
   * List of measurement co<nfiguration which are active in every UE attached to
  ...
  //test
  TracedCallback<uint64_t, uint16_t, uint16_t, LteRrcSap::PositionReport> m_recvPositionReportTrace;
  ...
  // Tracing the absolute values of x and y (latitude and longitude) errors applied to the UE position.
  TracedCallback<UePositionReportError> m_locationErrorTrace; // This is useless
  TracedCallback<UeCompletePositionReport> m_uePositionTrace;
  TracedCallback<std::map<Vector, std::pair<double, double>>> m_remErrorTrace;
  // This maps stores a boolean value for each physical position. If the position contains data, the entry is set to 1.
  TracedCallback<std::vector<Vector>> m_measRemNonEmptyEntries;
  ...
  std::pair<Vector, Ptr<LinkData>> FindNonEmptyRemEntry(const uint16_t servingBS, int uePosX, int uePosY);
  // Send best beams to gNB over X2 and UE over LTE link.
  void SendBeamsFromCoordinator(uint64_t imsi, size_t optimalEntryIndex, Ptr<LinkData> linkData);
  void HandoverToBestCell(uint64_t imsi, uint16_t targetCell);
  ...
  // Tracing of BeamIDs that would be set to gNB and UE for real and erroneous positions. 
  TracedCallback <RemBeamSelectionTraceParams> m_remBeamSelectionTrace;
  // Tracing of REM querying and recovery algorithm
  TracedCallback <RemQueryRecoveryAlgorithmTraceParams> m_remRecoveryAlgoTrace;
  void TraceBeamIdSelection(
      const uint64_t imsi,
      const uint16_t servingCellId,
      const Vector3D& realPosition,
      const Vector3D& errPosition,
      boost::optional<size_t> suboptimalBeamIndex = boost::none);

  // Pre-defined error values from an Ornstein-uhlenbeck process for medium urbanization
  std::vector<Vector> m_uePositionErrorValues;
  std::map<uint64_t, uint64_t> m_perImsiPosReportCounter;


  // Estimation of SINR for a given REM entry
