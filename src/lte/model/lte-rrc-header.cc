...
//test
void
RrcAsn1Header::SerializePositionReport (LteRrcSap::PositionReport report) const
{
  // FIXME: how to serialize double values??
  std::cout << "RrcAsn1Header::SerializePositionReport" << std::endl;
  SerializeInteger(static_cast<int>(report.imsi), 0, 2147483647);
  SerializeInteger(static_cast<int>(report.cellId), 0, 65535);
  SerializeInteger(static_cast<int>(report.uePosition.x * 1000), 0, 2147483647);
  SerializeInteger(static_cast<int>(report.uePosition.y * 1000), 0, 2147483647);
  SerializeInteger(static_cast<int>(report.uePosition.z * 1000), 0, 2147483647);
}
...
Buffer::Iterator
RrcAsn1Header::DeserializePositionReport (LteRrcSap::PositionReport *report, Buffer::Iterator bIterator)
{
  int n;
  bIterator = DeserializeInteger(&n, 0, 2147483647, bIterator);
  report->imsi = static_cast<uint64_t>(n);
  bIterator = DeserializeInteger(&n, 0, 65535, bIterator);
  report->cellId = static_cast<uint16_t>(n);
  std::cout << "RrcAsn1Header::DeserializePositionReport: n for cell id is " << n << std::endl;
  bIterator = DeserializeInteger(&n, 0, 2147483647, bIterator);
  report->uePosition.x = n / 1000;
  bIterator = DeserializeInteger(&n, 0, 2147483647, bIterator);
  report->uePosition.y = n / 1000;
  bIterator = DeserializeInteger(&n, 0, 2147483647, bIterator);
  report->uePosition.z = n / 1000;
...
//////////////////// PositionReportHeader class ////////////////////////

PositionReportHeader::PositionReportHeader ()
{
}
...
void
PositionReportHeader::PreSerialize() const
{
  m_serializationResult = Buffer ();
  // Serialize DCCH message
  SerializeUlDcchMessage (1);
  SerializePositionReport(m_positionReport);
  FinalizeSerialization ();
...
uint32_t
PositionReportHeader::Deserialize (Buffer::Iterator bIterator)
{
  std::bitset<0> bitset0;
  bIterator = DeserializeSequence (&bitset0,false,bIterator);
  bIterator = DeserializeUlDcchMessage (bIterator);
  bIterator = DeserializePositionReport(&m_positionReport, bIterator);

  return GetSerializedSize ();
}

void
PositionReportHeader::Print(std::ostream &os) const
{
  os << "x: " << m_positionReport.uePosition.x << std::endl;
  os << "y: " << m_positionReport.uePosition.y << std::endl;
  os << "z: " << m_positionReport.uePosition.z << std::endl;
}

void
PositionReportHeader::SetMessage (LteRrcSap::PositionReport msg)
{
  m_positionReport.uePosition.x = msg.uePosition.x;
  m_positionReport.uePosition.y = msg.uePosition.y;
  m_positionReport.uePosition.z = msg.uePosition.z;

  m_isDataSerialized = false;
}

LteRrcSap::PositionReport
PositionReportHeader::GetMessage () const
{
  LteRrcSap::PositionReport msg;
  msg = m_positionReport;
  return msg;
}
