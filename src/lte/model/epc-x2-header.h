...
//test
class EpcX2UePositionReport : public Header
{
public:
  EpcX2UePositionReport();
  virtual ~EpcX2UePositionReport();
  ...
  uint64_t GetUeImsi () const;
  void SetUeImsi (uint64_t ueImsi);
  void SetPositionReport(LteRrcSap::PositionReport report);
  void SetTargetCellId(uint16_t targetCell);

  uint32_t GetLengthOfIes () const;
  uint32_t GetNumberOfIes () const;
  LteRrcSap::PositionReport GetPositionReport() const;
  uint16_t GetCellId() const; // Source cell id

private:
  uint32_t          m_numberOfIes; ///< number of IEs. WTF is this? Information Element?
  uint32_t          m_headerLength; ///< header length

  uint64_t m_imsi;
  uint16_t m_targetCellId;
  // uint16_t cellId; // source cell ID
  LteRrcSap::PositionReport m_report;
};

// test: for commanding the desired beam to the target gNB
class EpcX2LinkDataToGnbHeader : public Header
{
public:
  EpcX2LinkDataToGnbHeader();
  virtual ~EpcX2LinkDataToGnbHeader();
  ...
  void SetUeImsi (uint64_t ueImsi);
  void SetTargetCellId(uint16_t targetCell);
  void SetLinkData(EpcX2Sap::LinkDataToGnb linkData);

  uint64_t GetUeImsi () const;
  uint32_t GetLengthOfIes () const;
  uint32_t GetNumberOfIes () const;
  LteRrcSap::LinkData GetLinkData() const;
