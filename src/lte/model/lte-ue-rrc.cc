...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddAttribute ("MaxRateThreshold",
                    ...
                    MakeDoubleChecker <double> (-1000.0, 1000.0))
    .AddAttribute ("PositionReportPeriodSeconds",
                    "Period in which the UE sends its Position Report [s]",
                    DoubleValue (1.),
                    MakeDoubleAccessor(&LteUeRrc::m_positionReportPeriodSeconds),
...
LteUeRrc::DoSendData (Ptr<Packet> packet, uint8_t bid)
{
  ...
  std::cout << "LteUeRrc::DoSendData sends a packet to addr " << packet->GetDAddr() << std::endl;
...
LteUeRrc::DoReceivePdcpSdu (LtePdcpSapUser::ReceivePdcpSduParameters params)
{
  ...
  //std::cout << "LteUeRrc::DoReceivePdcpSdu: PdcpSDU logical channel id " << static_cast<int>(params.lcid) << std::endl;
}
...
{
  NS_LOG_FUNCTION (this << " RNTI " << m_rnti);
  NS_LOG_FUNCTION (this << " m_state " << m_state);
...
{
  NS_LOG_FUNCTION (this << (uint16_t) measId);
  std::cout << "LteUeRrc::SendMeasurementReport is called" << std::endl; 
...
LteUeRrc::RadioLinkFailureDetected ()
{
  ...
  // If we did not receive a sweep trigger from RRC
  if (!m_sweepTriggerFromRrcReceived)
  {
    // and there are not handover events for this imsi (that are not due to RLF)
    ...
    if (!DoSendCheckForHandoverEvents (m_imsi))
    {
      ...
      if (m_cphySapProvider.at(0)->GetLastPerceivedSinr() < m_maxRateThreshold)
      {
        ...
        if (params.snrBeforeSweep < m_outageThreshold)
        {
          ...
          // We are in outage. If we were connected to a cell, deregister from it
          if (m_cellId != 0)
          {
            DoSendUeDeregisterToGnb();
          }
          // If we were not attached to anything, then sschedule a beam sweep if not done already
          else if (m_cellId == 0)
          {
        ...
        // if we are not below the outage threshold, then 
        else
        {
          ...
          if (DynamicCast<NrUeNetDevice>(m_cphySapProvider.at(0)->GetNetDeviceFromPhy()) != nullptr)
          {
            ...
            // we are not sweeping, will schedule a sweep now
            else
            {
      ...
      // We are no longer below RLF threshold
      else
      {
  ...
  // we have a triger from RRC for sweeping
  else
  {
    ...
    // LAMM: this is probably a place where we need to use REM-based beam selection instead of sweeping
    // If we are not doing a handover now
    if (!DoSendCheckForHandoverEvents (m_imsi))
    {
      BeamSweepTraceParams params;
      params.imsi = m_imsi;
...
LteUeRrc::DoSendOptimalBeamMapToLteCoordinator (std::map<uint8_t, std::vector<std::pair<std::pair<SfnSf, uint16_t>, std::pair<double, BeamId>>>> cellOptimalBeamMap)
{
  ...
  std::cout << "LteUeRrc::DoSendOptimalBeamMapToLteCoordinator: calling SendOptimalGnbBeamMap via nr rrc protocol ideal" << std::endl;
...
void
LteUeRrc::DoRecvRemBeamFromLTECoordinator(LteRrcSap::LinkData linkData)
{
  std::cout << "UE received REM beam command from LTE CO" << std::endl;
  // Forward to NrUePhy
  m_cphySapProvider.at(0)->SetRemBeam(linkData);
}
...
{
  std::cout << "LteUeRrc::DoIsRrcIdleStart: state is " << GetState() << std::endl;
  if (GetState () == IDLE_RANDOM_ACCESS) // WTF, this is not IDLE_START
  {
    return true;
  }
  else
...
void
LteUeRrc::SendPositionReport(Vector3D position)
{
  ...
  report.imsi = GetImsi();
  report.cellId = GetCellId(); // This is the source cell ID
  report.uePosition.x = position.x;
...
// Sends the SSB beam report to the gNB over LTE? Not to CO, as it uses m_rrcSapUser
void 
