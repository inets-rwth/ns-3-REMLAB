...
//****************************************************************************************************
NS_OBJECT_ENSURE_REGISTERED (EpcX2UePositionReport);

EpcX2UePositionReport::EpcX2UePositionReport()
  : m_numberOfIes (1 + 1 + 1 + 1 +1),
    m_headerLength (8 + 2 + + 8 + 2 + 8 + 8 + 8)
{
...
TypeId
EpcX2UePositionReport::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EpcX2UePositionReport")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor <EpcX2UePositionReport> ()
  ;
  return tid;
}

TypeId
EpcX2UePositionReport::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
EpcX2UePositionReport::GetSerializedSize (void) const
{
  return m_headerLength;
}

void 
EpcX2UePositionReport::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;

  i.WriteHtonU64 (m_imsi);
  i.WriteHtonU16 (m_targetCellId);
  i.WriteHtonU64 (m_report.imsi);
  i.WriteHtonU16 (m_report.cellId);
  i.WriteHtonU64 (m_report.uePosition.x);
  i.WriteHtonU64 (m_report.uePosition.y);
  i.WriteHtonU64 (m_report.uePosition.z);
}

uint32_t
EpcX2UePositionReport::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  ...
  m_report.imsi = i.ReadNtohU64 ();
  m_headerLength = +8;
  m_numberOfIes++;

  m_report.cellId = i.ReadNtohU16 ();
  m_headerLength = +2;
  m_numberOfIes++;

  m_report.uePosition.x = i.ReadNtohU64 ();
  m_headerLength = +8;
  m_numberOfIes++;

  m_report.uePosition.y = i.ReadNtohU64 ();
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?

  m_report.uePosition.z = i.ReadNtohU64 ();
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?
...
void
EpcX2UePositionReport::SetPositionReport(LteRrcSap::PositionReport report)
{
  m_report.imsi = report.imsi;
  m_report.cellId = report.cellId;
  m_report.uePosition.x = report.uePosition.x;
  m_report.uePosition.y = report.uePosition.y;
  m_report.uePosition.z = report.uePosition.z;
}

void
EpcX2UePositionReport::SetTargetCellId(uint16_t targetCell)
{
  m_targetCellId = targetCell;
}

void
EpcX2UePositionReport::SetUeImsi(uint64_t ueImsi)
{
  m_imsi = ueImsi;
}

void 
EpcX2UePositionReport::Print (std::ostream &os) const
{
  os << "UE IMSI " << m_imsi << " Target Cell ID " << m_targetCellId << " Position:\n";
  os << " x:" << m_report.uePosition.x << std::endl;
  os << " y:" << m_report.uePosition.y << std::endl;
  os << " z:" << m_report.uePosition.z << std::endl;
}

uint32_t
EpcX2UePositionReport::GetLengthOfIes () const
{
  return m_headerLength;
}

uint32_t
EpcX2UePositionReport::GetNumberOfIes () const
{
  return m_numberOfIes;
}

LteRrcSap::PositionReport
EpcX2UePositionReport::GetPositionReport() const
{
  return m_report;
}
...
NS_OBJECT_ENSURE_REGISTERED (EpcX2LinkDataToGnbHeader);

EpcX2LinkDataToGnbHeader::EpcX2LinkDataToGnbHeader()
  : m_numberOfIes (1 + 1 + 7),
    m_headerLength (8 + 2 + 56)
{
...
TypeId
EpcX2LinkDataToGnbHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EpcX2LinkDataToGnbHeader")
    .SetParent<Header> ()
    .SetGroupName("Lte")
    .AddConstructor <EpcX2LinkDataToGnbHeader> ()
  ;
  return tid;
}

TypeId
EpcX2LinkDataToGnbHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

uint32_t
EpcX2LinkDataToGnbHeader::GetSerializedSize (void) const
{
  return m_headerLength;
}

void 
EpcX2LinkDataToGnbHeader::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
  ...
  i.WriteHtonU16 (m_linkData.imsi);
  i.WriteHtonU64 (pack754(m_linkData.bestTxId));
  i.WriteHtonU64 (pack754(m_linkData.bestAoDElevation));
  i.WriteHtonU64 (pack754(m_linkData.bestAoDAzimuth));
  i.WriteHtonU64 (pack754(m_linkData.bestAoAElevation));
  i.WriteHtonU64 (pack754(m_linkData.bestAoAAzimuth));
  i.WriteHtonU64 (pack754(m_linkData.bestPathloss)); 
}

uint32_t
EpcX2LinkDataToGnbHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  ...
  m_linkData.imsi = i.ReadNtohU16 ();
  m_headerLength = +8;
  m_numberOfIes++;

  m_linkData.bestTxId = unpack754(i.ReadNtohU64 ());
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?

  // FIXME: all negative doubles are deserialized incorrectly
  m_linkData.bestAoDElevation = unpack754(i.ReadNtohU64 ());
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?

  m_linkData.bestAoDAzimuth = unpack754(i.ReadNtohU64 ());
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?

  m_linkData.bestAoAElevation = unpack754(i.ReadNtohU64 ());
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?

  m_linkData.bestAoAAzimuth = unpack754(i.ReadNtohU64 ());
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?

  m_linkData.bestPathloss = unpack754(i.ReadNtohU64 ());
  m_headerLength = +8;
  m_numberOfIes++; // FIXME? Is this needed? Do we count subfields of a member struct as IE?
...
void
EpcX2LinkDataToGnbHeader::SetLinkData(EpcX2Sap::LinkDataToGnb linkData)
{
  m_linkData.imsi = linkData.imsi;
  m_linkData.bestTxId = linkData.linkData.bestTxId;
  m_linkData.bestAoDElevation = linkData.linkData.bestAoDElevation;
  m_linkData.bestAoDAzimuth = linkData.linkData.bestAoDAzimuth;
  m_linkData.bestAoAElevation = linkData.linkData.bestAoAElevation;
  m_linkData.bestAoAAzimuth = linkData.linkData.bestAoAAzimuth;
  m_linkData.bestPathloss = linkData.linkData.bestPathloss;
}

void
EpcX2LinkDataToGnbHeader::SetTargetCellId(uint16_t targetCell)
{
  m_targetCellId = targetCell;
  // FIXME: might need to set this field of all underlying members?
}

void
EpcX2LinkDataToGnbHeader::SetUeImsi(uint64_t ueImsi)
{
  m_imsi = ueImsi;
  m_linkData.imsi = ueImsi;
}

void 
EpcX2LinkDataToGnbHeader::Print (std::ostream &os) const
{
  // TODO
  os << "EpcX2LinkDataToGnbHeader: IMSI: "<< m_imsi << ". Best gNB: " << m_linkData.bestTxId << std::endl;
}

uint32_t
EpcX2LinkDataToGnbHeader::GetLengthOfIes () const
{
  return m_headerLength;
}

uint32_t
EpcX2LinkDataToGnbHeader::GetNumberOfIes () const
{
  return m_numberOfIes;
}

LteRrcSap::LinkData
EpcX2LinkDataToGnbHeader::GetLinkData() const
{
  return m_linkData;
}

uint64_t 
EpcX2LinkDataToGnbHeader::pack754(long double f)
{
  uint16_t bits = 64;
  uint16_t expbits = 11;
  long double fnorm;
...
long double 
EpcX2LinkDataToGnbHeader::unpack754(uint64_t i)
{
  uint16_t bits = 64;
  uint16_t expbits = 11;
  long double result;
