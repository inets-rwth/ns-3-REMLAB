...
uint32_t LtePdcpHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (start.GetRemainingSize() != 1430)
    std::cout << "LtePdcpHeader::Deserialize: Buffer size before Deserialize: " << start.GetRemainingSize() << std::endl;
  // FIXME: this should not be needed? some error in above layer causes 'start' to have only 1 byte
  // but all other packets are 1430 bytes big
  if (start.GetRemainingSize() == 1)
  {
    std::cout << "!!!! LtePdcpHeader::Deserialize: WOULD HAVE CRASHED HERE!!!"<< std::endl;
    return 0; 
  }
