...
public:
  ...
  //test
  // TODO: optimize this. Do not want to copy REM on each call
  std::map<Vector, Ptr<LinkData>> GetAllLinkData() { return allLinkData; }
...
private:
  ...
  */
  ...
  void LoadInventoryWirelessInSite (std::string inventory_dir);
  void LoadLinkInfo (std::string link_file);

  // Alternative invenotry from the Wireless InSite Ray Tracer
  void ParseFspl(const std::string &filePath, uint8_t skipLines, std::function<void(std::istringstream &)> parser);
  void ParseDelaySpread(const std::string &filePath, uint8_t skipLines, std::function<void(std::istringstream &)> parser);
  void ParseDelaySpread(const std::string &filePath, uint8_t skipLines);
  void ParseLos(const std::string &filePath, uint8_t skipLines);
  /// Load DoD or DoA data
  void ParseDoX(const std::string &filePath, uint8_t skipLines, std::function<void(std::istringstream &, Vector &)> parser);
  std::string GenerateFilename(const std::string &inventory_dir, const std::string &prefix, uint32_t gnb);
  /// Stores mapping between entry index and position for Wireless InSite REM
  std::map<uint32_t, Vector> m_WirelessInSiteIndexPositionMap;
  void WriteRemToFile(const std::string& filename, const std::map<Vector, Ptr<LinkData>>& linkData);
  bool firstRemFile{true};
  /// Stores LOS data from Wireless InSite ray tracer. Each map key is a line, and the value is all columns.
  /// 0: no data, 1: LoS, -1: no LOS
  std::map<Vector, double> m_wiisLos;
  std::map<Vector, double> m_wiisDelaySpread;
  std::map<Vector, double> m_wiisPathLoss;
  ...
  /// Defines the amount of gNBs that are used in the simulation.
  uint64_t m_amountOfGnbs{0};
  /// Transmission power of each gNB in the simulation.
  double m_txPower{15.};
  uint64_t m_xPosRemOffset{375};
  uint64_t m_yPosRemOffset{376};
  uint64_t m_zPosRemOffset{0};
};
} // namespace ns3
